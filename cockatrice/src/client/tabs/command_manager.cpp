...
bool CommandManager::executeCommand(DeckCommandPtr command)
{
    ...
    m_redoStack = std::stack<DeckCommandPtr>{};
    ...
    // Emit signals
    emit commandExecuted(m_undoStack.empty() ? QString() : m_undoStack.top()->getDescription());
...
{
    m_undoStack = std::stack<DeckCommandPtr>{};
    m_redoStack = std::stack<DeckCommandPtr>{};
